...
from kroz.questions.week02 import FreeMemory, NewYearFuture, OsRelease, WhatsUname, questions
...
def main():
    ...
    app.show("""
             ...
             commands. To complete this section you shoul either have another 
             terminal open or use the ctrl-s key to escape the lab to the shell.              
             
             """, classes="welcome")
    
    app.ask(FreeMemory(key='total', points=5, checkpoint=True))
    app.ask(WhatsUname(key=WhatsUname.Keys.KERNEL_VERSION, points=5, checkpoint=True))
